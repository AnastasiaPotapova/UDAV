...
class Engine(QObject):
    ...
    # ВАЖНО: порядок имён в списке = порядок битов в байте payload (см. protocol.json,
    # bits в du16/electro_valves). Реальное поведение контроллера показало, что
    # исходный порядок (V1,V3,V6,V7 / V4,V5,V8) не совпадает с фактической разводкой:
    # нажатие V1 физически открывало V3 и наоборот, а в группе электроклапанов
    # нажатие V4/V5/V8 сдвигало реакцию на один клапан (V4->V8, V5->V4, V8->V5).
    # Порядок ниже подобран по факту реального открытия клапанов и совпадает с
    # порядком меток на схеме (ShematicWindow._build_scene, коммит "Внесение правок 4.0").
    DU16_VALVES = ["V1", "V3"]
    ELECTRO_VALVES = ["V5", "V8", "V4"]
